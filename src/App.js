...
function generaTesto(f) {
  ...
  // Costruzione frase — "di proprietà di Nome Cognome" senza articolo né titolo
  const testo =
    `A causa ${artCausa}${causa} nel punto di ${punto} ubicato ` +
    `${prepUbi} ${ubicazione} dell'appartamento interno ${intDann} di proprietà di ` +
    `${nomeDann}` +
    `${telDann ? " tel. " + telDann.replace(/\s+/g, "") : ""} ` +
    `${consInfo.frase} che ${verboDanneggiare} il sottostante ` +
    `appartamento interno ${intDato} di proprietà di ${nomeDato}` +
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  <TextField
                    ...
                    placeholder="Es. giunto del tubo di scarico"
                    value={form.punto}
