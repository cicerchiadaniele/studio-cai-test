...
export default function CondoMessenger() {
  ...
  const categorie = [
    ...
    { value: "Interventi di Manutenzione", icon: Building2, desc: "Segnala necessità di manutenzione" },
  ...
  const progress = useMemo(() => {
    let filled = 0;
    const fields = ["condominio", "nome", "email", "telefono", "categoria", "messaggio"];
    fields.forEach(f => { if (form[f]) filled++; });
    if (form.categoria === "Interventi di Manutenzione" && form.subcategoria) filled++;
    if (form.categoria === "Invio Documenti" && form.subcategoria && form.file) filled++;
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-neutral-700">Completamento modulo</span>
            <span className="text-sm font-semibold text-[var(--brand)]">{progress}%</span>
          </div>
          <div className="h-2 bg-neutral-100 rounded-full overflow-hidden">
            <motion.div
              className="h-full bg-gradient-to-r from-[var(--brand)] to-[var(--brand)]/80"
              initial={{ width: 0 }}
...
function TextField({ label, value, onChange, placeholder, type = "text", icon, required, error }) {
  return (
    <div>
      <label className="text-sm font-semibold text-neutral-700 flex items-center gap-2 mb-2">
        {icon}
          ...
          >
            <div className="flex items-start gap-3">
              <div className={cn(
                ...
                isSelected ? "bg-[var(--brand)] text-white" : "bg-neutral-100 text-neutral-600"
              )}>
              ...
              <div className="flex-1 min-w-0">
                <div className={cn(
                  ...
                  isSelected ? "text-[var(--brand)]" : "text-neutral-900"
                )}>
                  {cat.value}
                </div>
      ...
      {!file ? (
        ...
        >
          <div className="flex flex-col items-center justify-center pt-5 pb-6">
            ...
            <p className="text-xs text-neutral-500 mt-1">PDF, JPG, JPEG o PNG (max 10MB)</p>
          </div>
          <input
            ref={inputRef}
            type="file"
