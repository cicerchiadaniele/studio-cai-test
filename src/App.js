...
import {
  ...
  FileText, Users, Paperclip, Trash2, Info
} from "lucide-react";
...
/** Input destinatario libero con avviso parole chiave */
function DestinatarioInput({ id, value, onChange, error, disabled }) {
  return (
    <div>
      <input
        id={id}
        ...
        value={value}
        onChange={(e) => onChange(sanitizeCondominio(e.target.value))}
        placeholder="Es. Frateily, Rossi Impianti, Consiglio…"
        maxLength={80}
        disabled={disabled}
        className={cn(
          "w-full rounded-xl border bg-white px-4 py-3 text-sm text-neutral-900 placeholder:text-neutral-400",
      ...
      <div className="flex items-start gap-1.5 mt-2 px-1">
        ...
        <p className="text-[11px] text-amber-700 leading-snug">
          Usa la parola chiave esatta riconosciuta dal sistema di smistamento, altrimenti l'invio non andrà a buon fine.
        </p>
      </div>
      <FieldError msg={error} />
    </div>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (pdfFile) {
        ...
        fd.append("destinatario", form.destinatario.trim());
          ...
          <div className="bg-white rounded-2xl shadow-lift p-6 sm:p-8 space-y-6">
            ...
            <div className="pb-1">
              ...
              <p className="text-xs text-neutral-500">
                Compila i campi obbligatori e premi <strong>Invia</strong>.
              </p>
            </div>
