...
export default function AppAnagrafe() {
  ...
  const err = (f) => {
    ...
    const v = form[f];
    const req = ["condominio","nome","comuneResidenza","indirizzoResidenza","luogoNascita","dataNascita",
  ...
  const touchStep = (s) => {
    const t = {};
    if (s===1) { t["condominio"]=true; form.unita.forEach((_,i)=>{ ["destinazione","destinazioneAltro"].forEach(f=>{t[`u_${i}_${f}`]=true;}); });
              ...
              <motion.div key={step}
                ...
                {step===1 && (
                  <div className="space-y-5">
                    ...
                    <TF label="Condominio" required value={form.condominio} onChange={(v)=>upd("condominio",v)}
                      placeholder="Es. Via Marco Polo 84, Roma" icon={<Building2 className="w-4 h-4"/>}
                      error={err("condominio")} hint="Nome e/o indirizzo del condominio di riferimento" sanitizerKey="indirizzo"/>
