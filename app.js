...
function renderHistory(){
  ...
  h.forEach(row => {
    ...
    if(row.status === "held"){
      ...
      btn.addEventListener("click", () => retryHeld(row.request_id));
...
function reorderFields(){
  // La tipologia decide quali campi servono: va scelta per prima.
  // Va messa PRIMA del codice dipendente, non tra codice e data: occupa
  // due colonne (field--span2) e in mezzo lascerebbe una cella vuota.
...
function cleanHeaderBadge(){
  ...
  [...pill.childNodes].forEach(node => {
    if(node.nodeType === Node.TEXT_NODE && emoji.test(node.textContent)){
      node.textContent = node.textContent.replace(new RegExp(emoji, "gu"), "").trim();
