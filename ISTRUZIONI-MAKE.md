...
Modulo **Webhooks › Custom webhook**. Copia l'indirizzo che ti dà: va messo in
`config.json`, nel campo `webhook_url`.
...
Filtro: `parola_chiave` **not equal to** `quelramodellagodicomo`
...
spazi né accenti), quindi il confronto con `quelramodellagodicomo` è esatto
...
Filtro: `azione` **equal to** `link` **AND** `parola_chiave` equal to `quelramodellagodicomo`
